...
def float_checker(text):
    float_ = ""
    while float_ == "":  # countiues till got float
        try:
...
def add_combo():
    ...
    for i in range(0,combo_size):
        item_name = easygui.enterbox(f"what is item number {i+1}?")#getting a item
        item_price = float_checker(f"how much does {item_name} cost?")#getting it's price
...
def search_menu():
    search = easygui.enterbox("please enter combo name")#finding out what thier after
...
def delete():
    delete = easygui.enterbox("what input do you want to delete?")#getting what do you want removed
    if delete in menus:#if it's thier
        menus.pop(delete)#deleate it
        easygui.msgbox(f"{delete} deleted") #tell them you have
    else: # other wise
        easygui.msgbox(f"{delete} not found could not delete")#tell them you can't find it and return to menu
...
for i in range(0,len(options)):
    if action == options[i]:#checks if action choosen
